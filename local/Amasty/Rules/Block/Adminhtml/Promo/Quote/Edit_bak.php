    ...
    public function __construct()
    {
        ...
        $this->_formScripts[] = "
			function ampromo_note() {
                ...
                var noteTemplate = new Template('" . $this->__('Please see <a target="_blank" href="https://amasty.com/knowledge-base/special-promotions-#{value}.html?utm_source=extension&utm_medium=link&utm_campaign=userguide_sp_m1_#{value}">usage example</a>') . ".');
			...
			function amrules_hint() {
			    ...
			    if ($('rule_price_selector_note'))
			        $('rule_price_selector_note').update('');
			    
			    switch($('rule_price_selector').value) {
			        case '" . Amasty_Rules_Model_Observer::BASED_ON_BIGGEST . "':
			            $('note_price_selector').show();
			            break;
			        case '" . Amasty_Rules_Model_Observer::BASED_ON_ORIGINAL . "':
			            $('rule_price_selector').insert ({
                            after : new Element('p', {class: 'note', id: 'rule_price_selector_note'})
                        });
                        $('rule_price_selector_note').update('Notice: please consider - this setting doesn\'t function for Bundle Product with dynamic price.');
			            $('note_price_selector').hide();
			            break;
			        default:
			            break;
...
//                    $('rule_apply_to_shipping').up().up().hide();
				...
				if  ('ampromo_items' == $('rule_simple_action').value
                    ...
                    || 'ampromo_eachn' == $('rule_simple_action').value
                    ){
                        $('rule_amskip_rule').up().up().hide();
                        $('rule_price_selector').up().up().hide();
                        hideMaxDiscountField();
