...
class Amasty_PromoBannersLite_Model_Banners extends Mage_Core_Model_Abstract
{
    ...
    public function getRulesCollection()
    {
         ...
         */
        $rulesCollection = method_exists($rulesCollection, 'addWebsiteGroupDateFilter')
            ? $rulesCollection->addWebsiteGroupDateFilter($websiteId, $groupId)
            : $this->addWebsiteGroupDateFilter($rulesCollection, $websiteId, $groupId);
     ...
     * @return mixed
     */
    public function addWebsiteGroupDateFilter($collection, $websiteId, $customerGroupId, $now = null)
    {
        $collection->getSelect()->where('is_active=1');
        $collection->getSelect()->where('find_in_set(?, website_ids)', (int)$websiteId);
        $collection->getSelect()->where('find_in_set(?, customer_group_ids)', (int)$customerGroupId);
