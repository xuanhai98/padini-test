            ...
            if ($this->_currentAttributes)
            {
                ...
                foreach ($simpleProducts as $simple)
                {
                    ...
                    if ($key)
                    {
						...
						$inStock = Mage::getModel('cataloginventory/stock_item')->loadByProduct($simple)->getIsInStock();
                        /* old code */
						/* $confData[$strKey]['not_is_in_stock' ]  = !$simple->isSaleable(); */
                        /* new fix code */
						$confData[$strKey]['not_is_in_stock' ]  = !$inStock;
						
						/* fixcode for out-of-stock */
                        $confData[$strKey]['urlproduct' ]  = $this->getUrl('amconf/ajax/status', array('id' => $simple->getId()));
